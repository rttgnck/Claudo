...
export const TRACKS = [
  ...
  { id: 'bass', name: 'Bass', kind: 'synth', color: '#4ade80', poly: false, octave: -1, rows: 8 },
...
// Frequency for a synth track's pitch index, given key + scale.
...
export function noteName(track, pitchIndex, keyIdx, scaleSteps) {
  const root = 48 + keyIdx + (track.octave || 0) * 12;
  const m = Math.round(root + degreeSemis(scaleSteps, pitchIndex));
  return KEYS[((m % 12) + 12) % 12] + (Math.floor(m / 12) - 1);
}

// Default synth parameters per synth track.
export function defaultSynthParams(id) {
  if (id === 'bass') return { wave: 'sawtooth', attack: 0.005, decay: 0.18, sustain: 0.5, release: 0.14, cutoff: 900, reso: 6, glide: 0.04, drive: 0.2 };
  return { wave: 'square', attack: 0.005, decay: 0.22, sustain: 0.35, release: 0.28, cutoff: 3200, reso: 5, glide: 0, drive: 0.1 };
}
